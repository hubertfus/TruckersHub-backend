...
router.get("/", async (req, res) => {
  try {
    ...
    if (vehicleId)
      orConditions.push({ _id: new mongoose.Types.ObjectId(vehicleId) });

    const matchCondition = orConditions.length > 0 ? { $or: orConditions } : {};
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching the vehicles." });
  }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "An error occurred while fetching the vehicle." });
  }
});
